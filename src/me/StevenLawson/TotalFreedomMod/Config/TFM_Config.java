...
public class TFM_Config extends YamlConfiguration // BukkitLib @ https://github.com/Pravian/BukkitLib
{
    ...
    public void save()
    {
        ...
        {
            plugin.getLogger().log(Level.SEVERE, "Could not save configuration file: {0}", configFile.getName());
    ...
    public void load()
    {
        try
        {
            if (copyDefaults)
            {
                if (!configFile.exists())
                {
                    ...
                    {
                        plugin.getLogger().log(Level.SEVERE, "Could not write default configuration file: {0}", configFile.getName());
                        plugin.getLogger().severe(ExceptionUtils.getStackTrace(ex));
                    }
                    plugin.getLogger().log(Level.INFO, "Installed default configuration {0}", configFile.getName());
                }
        ...
        {
            plugin.getLogger().log(Level.SEVERE, "Could not load configuration file: {0}", configFile.getName());
            plugin.getLogger().severe(ExceptionUtils.getStackTrace(ex));
        }
